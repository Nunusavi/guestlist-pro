...
import { Search, Users, CheckCircle, XCircle, Wifi, WifiOff, LogOut, Home, BarChart3, History, UserPlus, Download, AlertCircle, Trash2, Edit, X as XIcon } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { BarChart, Bar, LineChart, Line, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';
import { useAuth } from '../contexts/AuthContext';
import { useGuests } from '../hooks/useGuests';
import { useAdminStats, useAuditLog, useUserManagement, useExport } from '../hooks/useAdmin';
import { useToast } from '../components/Toast';

export default function AdminPage() {
  const { user, logout } = useAuth();
  const { addToast } = useToast();
  const { guests, loading: guestsLoading, fetchGuests, searchGuests: searchGuestsApi } = useGuests();
  const { stats, loading: statsLoading, refetch: refetchStats } = useAdminStats(true, 10000); // Auto-refresh every 10s
  const { auditLog, loading: auditLoading, fetchAuditLog } = useAuditLog();
  const { ushers, loading: ushersLoading, createUsher, updateUsher, deleteUsher } = useUserManagement();
  ...
  const handleDeleteUser = useCallback(async (usherId) => {
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white font-semibold">{user?.fullName || 'Admin'}</h2>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'guests' && (
            <GuestsTab
              ...
              onSelectGuest={(id) => setSelectedGuests(prev => prev.includes(id) ? prev.filter(gid => gid !== id) : [...prev, id])}
              onSelectAll={() => setSelectedGuests(selectedGuests.length === filteredGuests.length ? [] : filteredGuests.map(g => g.id))}
...
function DashboardTab({ stats, chartData, ticketTypeData, guests, loading }) {
  const recentCheckIns = guests.filter(g => g.status === 'Checked-In' && g.checkInTime)
    .sort((a, b) => new Date(b.checkInTime) - new Date(a.checkInTime))
    .slice(0, 10);

  const notCheckedIn = guests.filter(g => g.status === 'Not Checked-In');
  const percentage = stats?.checkInPercentage?.toFixed(1) || 0;
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Guests" value={stats?.totalGuests || 0} icon={<Users className="w-5 h-5" />} color="blue" />
        <StatCard label="Checked In" value={stats?.checkedInCount || 0} subtitle={`${percentage}%`} icon={<CheckCircle className="w-5 h-5" />} color="green" />
        <StatCard label="Not Arrived" value={(stats?.totalGuests || 0) - (stats?.checkedInCount || 0)} icon={<XCircle className="w-5 h-5" />} color="orange" />
        <StatCard label="Plus-Ones" value={`${stats?.plusOnesCheckedIn || 0}/${stats?.plusOnesAllowed || 0}`} icon={<UserPlus className="w-5 h-5" />} color="purple" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <PieChart>
              <Pie data={ticketTypeData} cx="50%" cy="50%" labelLine={false} label={({ name, value }) => `${name}: ${value}`} outerRadius={80} dataKey="value">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {recentCheckIns.length === 0 ? (
              ...
              recentCheckIns.map(guest => (
                <div key={guest.id} className="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                  ...
                  <span className={`px-2 py-1 rounded text-xs ${guest.ticketType === 'VIP' ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-blue-400'}`}>
                    {guest.ticketType}
                  </span>
                </div>
              ))
        ...
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
          <h3 className="text-white font-semibold mb-4">Awaiting Arrival ({notCheckedIn.length})</h3>
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {notCheckedIn.length === 0 ? (
              ...
              notCheckedIn.slice(0, 15).map(guest => (
                <div key={guest.id} className="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                  ...
                  <span className={`px-2 py-1 rounded text-xs ${guest.ticketType === 'VIP' ? 'bg-purple-500/20 text-purple-400' : 'bg-slate-600 text-slate-300'}`}>
                    {guest.ticketType}
                  </span>
                </div>
              ))
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        ...
        <div className={`p-2 rounded-lg bg-gradient-to-br ${colors[color]}`}>{icon}</div>
      </div>
...
function GuestsTab({ guests, selectedGuests, onSelectGuest, onSelectAll, searchTerm, onSearchChange, filterStatus, onFilterChange, sortBy, onSortChange, onExport, loading }) {
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-4">
      <div className="bg-slate-800 rounded-xl p-4 border border-slate-700 space-y-4">
        <div className="flex gap-4 flex-wrap">
          ...
          <select value={filterStatus} onChange={(e) => onFilterChange(e.target.value)} className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
            <option value="all">All ({guests.length})</option>
            <option value="checked-in">Checked In</option>
            <option value="not-checked-in">Not Checked In</option>
          </select>

          <select value={sortBy} onChange={(e) => onSortChange(e.target.value)} className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
            <option value="name">Sort by Name</option>
            <option value="checkInTime">Sort by Check-In Time</option>
          </select>

          <button onClick={onExport} className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors flex items-center gap-2">
        ...
        <AnimatePresence>
          {selectedGuests.length > 0 && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="flex items-center justify-between p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
              <span className="text-blue-400 font-medium">{selectedGuests.length} guest{selectedGuests.length > 1 ? 's' : ''} selected</span>
              <button className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition-colors text-sm">Clear</button>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
            ...
            <thead className="bg-slate-700/50 border-b border-slate-700">
              <tr>
                <th className="px-4 py-3 text-left">
                  <input type="checkbox" checked={selectedGuests.length === guests.length && guests.length > 0} onChange={onSelectAll} className="w-4 h-4 rounded bg-slate-700 border-slate-600" />
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Guest</th>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Plus-Ones</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Status</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Check-In Time</th>
              </tr>
            </thead>
              ...
              {guests.map(guest => (
                <tr key={guest.id} className="hover:bg-slate-700/30 transition-colors">
                  <td className="px-4 py-3">
                    <input type="checkbox" checked={selectedGuests.includes(guest.id)} onChange={() => onSelectGuest(guest.id)} className="w-4 h-4 rounded bg-slate-700 border-slate-600" />
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-white font-medium">{guest.firstName} {guest.lastName}</div>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${guest.ticketType === 'VIP' ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-blue-400'}`}>
                      {guest.ticketType}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span className="text-sm text-slate-400">{guest.checkInTime ? new Date(guest.checkInTime).toLocaleString() : '-'}</span>
                  </td>
                </tr>
              ))}
            </tbody>
...
function UsersTab({ ushers, currentUser, onCreateUser, onEditUser, onDeleteUser, loading }) {
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button onClick={onCreateUser} className="px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white rounded-lg transition-all flex items-center gap-2">
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-700">
              {ushers.map(usher => (
                <tr key={usher.id} className="hover:bg-slate-700/30 transition-colors">
                  <td className="px-4 py-3">
                    <div className="text-white font-medium">{usher.username}</div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-slate-300">{usher.fullName}</div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${usher.role === 'Admin' ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-blue-400'}`}>
                      {usher.role}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${usher.active ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                      {usher.active ? 'Active' : 'Inactive'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm text-slate-400">{usher.createdAt}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <button onClick={() => onEditUser(usher)} className="p-2 hover:bg-slate-700 rounded-lg transition-colors" title="Edit">
                      ...
                      <button 
                        ...
                        title={usher.id === currentUser.id ? "Can't delete yourself" : "Delete"}
                      >
        ...
        <div className="space-y-3">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-white font-medium">{loading ? 'Exporting...' : 'Export as CSV'}</p>
...
function UserModal({ user, onClose, onCreate, onUpdate, addToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        addToast(`User ${user ? 'updated' : 'created'} successfully`, 'success');
        onClose();
      } else {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-white">{user ? 'Edit User' : 'Create User'}</h2>
          <button onClick={onClose} className="p-2 hover:bg-slate-700 rounded-lg transition-colors">
            <XIcon className="w-5 h-5 text-slate-400" />
          </button>
        </div>
